...
elform.addEventListener("submit",(e)=>{
    ...
    fetch(`https://cooperative-outerwear-yak.cyclic.app/notes/update/${id}`,{
        method:'PATCH',
        headers:{
...
function fetchRender() {
    ...
    fetch("https://cooperative-outerwear-yak.cyclic.app/notes", {
        headers: {
            'Content-type': 'application/json',
            'authorization': `Bearer ${localStorage.getItem("token")}`
        }
...
function createCard(title, category, note,_id) {
    ...
    a2.addEventListener("click",()=>{
        fetch(`https://cooperative-outerwear-yak.cyclic.app/notes/delete/${_id}`,{
            method:"DELETE",
            headers:{
                'Content-type':'application/json',
